...
| subnets | Optional: A map containing subnet details Used to derive the subnetwork URI if subnetwork is not provided. | <pre>list(object({<br>    id            = string<br>    name          = optional(string)<br>    region        = string<br>    purpose       = string<br>    self_link     = optional(string)<br>    ip_cidr_range = optional(string)<br>    network       = optional(string)<br>  }))</pre> | `[]` | no |
